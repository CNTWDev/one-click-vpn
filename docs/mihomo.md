...
1. 在个人平台「创建连接」中选择 **Clash**，填写连接名称，区域可保留「自动选择」。用户无需选择底层协议。
2. 点击「创建并下载」，系统复用 WireGuard 服务生成 Mihomo 格式并开始下载。浏览器未保存时，可点击成功提示区的「重新下载」。已有连接也可在详情中选择下载客户端 Clash，无需重新创建。
...
个人端按客户端组织创建入口，Clash 与 WireGuard 共用 WireGuard 协议，OpenVPN 使用原生配置。证书、底层协议和配置历史收纳在「证书与配置详情」；停用、撤销等操作在「管理连接」中。下载客户端的临时选择不会改变服务端凭据身份或权限。
