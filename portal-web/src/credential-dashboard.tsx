...
export function CredentialDashboard({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  async function changeAccess(action: "enable" | "disable" | "delete") {
    if (!selected || !window.confirm(`${action === "enable" ? "启用" : action === "disable" ? "停用" : "删除"}「${selected.name}」？${action === "delete" ? "所有配置将永久失效，历史流量与审计保留。" : "节点同步后生效。"}`)) return;
    setBusy(true); setError("");
    try {
  ...
  async function bulkAccess(action: "disable" | "revoke") {
    if (!window.confirm(action === "disable" ? "停用自己的全部有效凭据？可以逐个重新启用。" : "永久撤销自己的全部凭据？所有副本都将失效，无法恢复。")) return;
    setBusy(true); setError("");
    try {
