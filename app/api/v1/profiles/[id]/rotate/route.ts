...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const regionalEndpoints = Array.isArray(oldProfile.protocol_payload.regionalEndpoints)
      ...
      : undefined;
    const next = await issueConnectionProfile({ actorUserId: user.id, credentialId: oldProfile.credential_id || undefined, deviceId: oldProfile.device_id, nodeId: oldProfile.node_id, protocol: oldProfile.protocol, transport: oldProfile.transport, rotateCredential: oldProfile.protocol === "openvpn", regionalEndpoints });
